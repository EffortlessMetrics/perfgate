...
- [ ] 1. Fix workspace dependency issues
  - [ ] 1.1 Add perfgate-adapters dependency to perfgate-cli Cargo.toml
    - Add `perfgate-adapters = { path = "../perfgate-adapters" }` to dependencies
    - _Requirements: 1.1, 2.1_
  
  - [ ] 1.2 Add serde dependency to xtask Cargo.toml
    - Add `serde.workspace = true` to xtask dependencies
    - _Requirements: 9.3_
  
  - [ ] 1.3 Verify workspace builds without errors
    - Run `cargo build --all`
    - _Requirements: 1.1_

- [ ] 2. Checkpoint - Verify build passes
  - Ensure `cargo build --all` succeeds, ask the user if questions arise.

- [ ] 3. Add property-based tests for domain logic
  - [ ] 3.1 Add proptest dependency to perfgate-domain
