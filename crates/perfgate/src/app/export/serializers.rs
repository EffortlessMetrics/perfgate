...
use super::escaping::{csv_escape, html_escape, prometheus_escape_label_value, write_opt_u64};
use super::{CompareExportRow, RunExportRow};

pub(crate) fn run_row_to_junit_run(
    receipt: &RunReceipt,
    _row: &RunExportRow,
...
pub(crate) fn run_row_to_csv(row: &RunExportRow) -> anyhow::Result<String> {
...
pub(crate) fn run_row_to_jsonl(row: &RunExportRow) -> anyhow::Result<String> {
...
pub(crate) fn compare_rows_to_csv(rows: &[CompareExportRow]) -> anyhow::Result<String> {
    let mut output = String::new();
...
pub(crate) fn compare_rows_to_jsonl(rows: &[CompareExportRow]) -> anyhow::Result<String> {
    let mut output = String::new();
...
pub(crate) fn run_row_to_html(row: &RunExportRow) -> anyhow::Result<String> {
    let html = format!(
...
pub(crate) fn compare_rows_to_html(rows: &[CompareExportRow]) -> anyhow::Result<String> {
...
pub(crate) fn run_row_to_prometheus(row: &RunExportRow) -> anyhow::Result<String> {
...
pub(crate) fn compare_rows_to_junit(
...
pub(crate) fn compare_rows_to_prometheus(rows: &[CompareExportRow]) -> anyhow::Result<String> {
    let mut out = String::new();
    for row in rows {
        let bench = prometheus_escape_label_value(&row.bench_name);
