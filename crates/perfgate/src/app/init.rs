...
2. Promote a trusted first run with `perfgate baseline promote --config {config} --bench <bench>`.
...
"#,
    ...
    fn onboarding_readme_mentions_artifacts_and_next_steps() {
        ...
        assert!(
            content.contains("perfgate baseline promote --config perfgate.toml --bench <bench>")
        );
    }
