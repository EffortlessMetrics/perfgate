//! CLI I/O helpers for local files and object-store-backed locations.
//!
//! This module keeps path resolution and JSON persistence concerns out of the
//! command dispatcher in `main.rs`. The CLI can ask for typed reads/writes
//! without knowing whether a path is local or a remote object-store URI.
