//! CLI value parsers and option validators.
//!
//! Keeping these parsing-only helpers out of `main.rs` makes the command
//! dispatcher focus on orchestration while this module owns clap-compatible
//! parsing and cross-option validation.
...
pub(crate) fn parse_key_val_string(s: &str) -> Result<(String, String), String> {
    let (k, v) = s
        .split_once('=')
...
pub(crate) fn parse_key_val_f64(s: &str) -> Result<(String, f64), String> {
    let (k, v) = s
        .split_once('=')
        .ok_or_else(|| "expected KEY=VALUE".to_string())?;
...
pub(crate) fn parse_noise_policy(s: &str) -> Result<perfgate_types::NoisePolicy, String> {
...
pub(crate) fn parse_flakiness_score(s: &str) -> Result<f64, String> {
    let score: f64 = s
        .parse()
...
pub(crate) fn parse_verdict_status(s: &str) -> Result<VerdictStatus, String> {
    match s.to_lowercase().as_str() {
...
pub(crate) fn parse_metric_status(s: &str) -> Result<MetricStatus, String> {
    match s.to_lowercase().as_str() {
...
pub(crate) fn parse_host_mismatch_policy(s: &str) -> Result<HostMismatchPolicy, String> {
    match s {
...
pub(crate) fn parse_aggregation_policy(s: &str) -> Result<AggregationPolicy, String> {
    match s {
...
pub(crate) fn parse_aggregate_weight_mode(s: &str) -> Result<AggregateWeightMode, String> {
    match s {
...
pub(crate) fn parse_weight_map(weights: &[String]) -> anyhow::Result<BTreeMap<String, f64>> {
...
pub(crate) fn validate_aggregate_options(
...
pub(crate) fn parse_significance_alpha(s: &str) -> Result<f64, String> {
...
pub(crate) fn normalize_paired_cli_command(
    args: Vec<String>,
    flag_name: &str,
