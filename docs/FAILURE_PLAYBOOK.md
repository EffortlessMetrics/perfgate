...
The Rust self-benchmark wrapper failed before it could evaluate the performance policy.

- **Check Logs**: Look for "perfgate binary not found" or `perfgate-selfbench perf-wrapper` errors.
- **Cause**: Usually due to changes in the project structure, broken build, or missing dependencies in the runner image.
- **Fix**: Verify the `perf-wrapper` workloads in `crates/perfgate-selfbench/src/main.rs` and the `.ci/perfgate-*.toml` command entries.
